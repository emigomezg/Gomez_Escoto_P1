...
#define SIGNAL_DATA_ARRAY_SIZE 256

#define ARRAY_INIT_VALUE 0

static uint8_t DAC0_array_index = ARRAY_INIT_VALUE;

static uint8_t DAC1_array_index = ARRAY_INIT_VALUE;

  /* The next array definition contains all the values that a sine
   * signal can take within a period, in hex. */

 uint16_t sine_signal_array[SIGNAL_DATA_ARRAY_SIZE] =
 {
 ...
 /* The next array definition contains all the values that a triangle
  * signal can take within a period, in hex. */

 uint16_t triangle_signal_array[SIGNAL_DATA_ARRAY_SIZE] =
 {
 ...
 /* The next array definition contains all the values that a square
   * signal can take within a period, in hex. */

uint16_t square_signal_array[SIGNAL_DATA_ARRAY_SIZE] =
{
...
{
	dac_data_out_t dac_data;

	if(SIGNAL_DATA_ARRAY_SIZE-1 == DAC0_array_index)
	{
		DAC0_array_index = ARRAY_INIT_VALUE;
	}

	if(wave_form == SINE)
	{
		dac_data.data_high = (uint8_t)(sine_signal_array[DAC0_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & sine_signal_array[DAC0_array_index]);
	}
	else if (wave_form == TRIANGLE)
	{
		dac_data.data_high = (uint8_t)(triangle_signal_array[DAC0_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & triangle_signal_array[DAC0_array_index]);
	}
	else if(wave_form == SQUARE)
	{
		dac_data.data_high = (uint8_t)(square_signal_array[DAC0_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & square_signal_array[DAC0_array_index]);
	}
	else
	{
		dac_data.data_high = 0;
		dac_data.data_low = 0;
	}

	DAC0_out_value(dac_data);
...
void DAC1_signal_creator(uint8_t wave_form)
{
	...
	if(SIGNAL_DATA_ARRAY_SIZE-1 == DAC1_array_index)
	{
		DAC0_array_index = ARRAY_INIT_VALUE;
	}
	...
	if(wave_form == SINE)
	{
		dac_data.data_high = (uint8_t)(sine_signal_array[DAC1_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & sine_signal_array[DAC1_array_index]);
	}
	else if (wave_form == TRIANGLE)
	{
		dac_data.data_high = (uint8_t)(triangle_signal_array[DAC1_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & triangle_signal_array[DAC1_array_index]);
	}
	else if(wave_form == SQUARE)
	{
		dac_data.data_high = (uint8_t)(square_signal_array[DAC1_array_index] >> 8);
		dac_data.data_low = (uint8_t)(0x00FF & square_signal_array[DAC1_array_index]);
	}
	else
	{
		dac_data.data_high = 0;
